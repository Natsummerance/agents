# TOOLS.md - Humanizer 去痕助手技术参考

> **文档说明**: 本文件记录 Humanizer 去痕助手的工具清单、使用规范、常见错误模式和最佳实践
...
- `read_file` — 读取文件内容（上限 512KB）
  ...
  - 示例：`read_file(path="books/blader-humanizer/book-overview.md", offset=0, limit=100)`

- `list_dir` — 列出目录内容（上限 200 条）
  - 参数：`path`(string, 必填)
  - 示例：`list_dir(path="books/blader-humanizer/candidates")`

- `grep` — 文本搜索（上限 100 条命中）
  - 参数：`pattern`(string, 必填), `path`(string, 可选), `file_pattern`(string, 可选)
  - 示例：`grep(pattern="AI 痕迹", path="books/blader-humanizer/", file_pattern="*.md")`

- `glob` — 文件模式匹配（上限 200）
  - 参数：`pattern`(string, 必填), `path`(string, 可选)
  - 示例：`glob(pattern="**/*.test.json", path="books/blader-humanizer/")`
...
- `ask_user` — 向用户提问
  ...
  - 示例：`ask_user(question="请确认源文本类型：是个人写作还是技术文档？")`

### WORKSPACE_WRITE 权限（10 个）

- `write_file` — 写入文件（UTF-8，自动创建父目录）
  - 参数：`path`(string, 必填), `content`(string, 必填)
  - 示例：`write_file(path="books/blader-humanizer/BOOK_OVERVIEW.md", content="# ...")`

- `edit_file` — 编辑文件（多 hunk 替换，原子性）
  - 参数：`path`(string, 必填), `edits`(array, 必填, 每项含 `old` 和 `new`)
  - 示例：`edit_file(path="skills/pattern-based-detection/SKILL.md", edits=[{old: "foo", new: "bar"}])`

- `mkdir` — 创建目录
  - 参数：`path`(string, 必填)
  - 示例：`mkdir(path="books/blader-humanizer/rejected")`

- `git_init` — 初始化 Git 仓库
  - 参数：`path`(string, 可选, 默认当前目录)
  - 示例：`git_init(path="./books/blader-humanizer")`

- `git_add` — Git 暂存文件
  - 参数：`path`(string, 必填, 文件或目录路径)
  - 示例：`git_add(path="books/blader-humanizer/")`

- `git_commit` — Git 提交
  - 参数：`message`(string, 必填)
  - 示例：`git_commit(message="feat: add pattern-based-detection skill")`
...
- `run_shell` — 执行 Shell 命令（需人工审批）
  ...
  - 示例：`run_shell(command="cp -r books/blader-humanizer/skills ~/.openclaw/skills/", cwd=".")`
...
- **skill-slug**：kebab-case，简短有意义
  - 好例：`pattern-based-detection`, `two-pass-rewriting`, `voice-matching`
  - 坏例：`skill-1`, `blader-humanizer-skill-001`
...
### 错误模式1：没有源文本就凭记忆蒸馏
**问题**: 用户说"帮我拆 blader/humanizer"，但没有提供文本
**解决方案**:
